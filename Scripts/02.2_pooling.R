## Pooling data sets (LUX and GS only)
...
covar = bind_rows(covar_lux, covar_gs) %>%
  select(Batch, Indiv.ID, Age, Gender, Family.ID, Country, Area, "Weight (mg)", "Length (cm)")
rownames(covar) = covar$Indiv.ID
str(covar)

ifelse(dir.exists("../Figures/Pooled2"),"",dir.create("../Figures/Pooled2"))
# Age
pdf("../Figures/Pooled2/Age_dist.pdf", width = 5, height = 5)
hist(covar$Age, main = "Pooled (LUX/GS)", xlab = "Age (years)")
dev.off()
...
{pdf("../Figures/Pooled2/Observed_prop.pdf", width=14, height=7)
  par(mar=c(7,4,9,1))
  plot(prop,
       col=ifelse(prop>0.5, mycolours, alpha(mycolours, 0.2)),
       xaxt="n", ylab="", xlab = "",
  ...
  for (k in 1:(length(xseqblack)-1)){
    axis(side=3, at=mean(xseqblack[c(k,k+1)]), line=0.2, tick=FALSE,
         labels=unique(family)[k], cex.axis = 0.7, las = 2, col.axis = family.colours[k])
  }
  dev.off()
}
...
{pdf("../Figures/Pooled2/Detection_prop.pdf", width=14, height=7)
  par(mar=c(7,4,9,1))
  plot(prop,
       col=ifelse(prop>0.1, mycolours, alpha(mycolours, 0.2)),
       xaxt="n", ylab="", xlab = "",
       type="n", lwd=2, ylim = c(0,1))
  xseqgreysep=c(min(xseq)-myspacing/2,apply(rbind(xseq[-1],xseq[-length(xseq)]),2,mean),max(xseq)+myspacing/2)
  if (background){
  ...
  plot(prop,
       ...
       xaxt="n", ylab="Proportion observed", xlab = "",
       type="h", lwd=2, ylim = c(0,1))
  abline(h=0.1,lty=2,col="darkred")
  for(i in 1:length(xseq)){
    axis(1, at=xseq[i], labels = mylabels[i], las=2, cex.axis = 0.5)}
  xseqblack=c(xseq[!duplicated(family)]-myspacing/2, max(xseq)+myspacing/2)
  abline(v=xseqblack,lty=3,col="black")
...
# Number of chemical compounds with 50% or more NA
nrow(chem[which(chem$NA_prop>=0.5),])

# Filter out >=50% NA
mat = mat[,which(chem$NA_prop<0.5)]
chem = chem[which(chem$NA_prop<0.5),]
...
# Save data sets
ifelse(dir.exists("../Processed/Pooled2"),"",dir.create("../Processed/Pooled2"))
saveRDS(covar, "../Processed/Pooled2/Participant_covariate_info_subset.rds")
saveRDS(chem, "../Processed/Pooled2/Chemical_compound_info_subset.rds")
saveRDS(mat, "../Processed/Pooled2/Chemical_compound_matrix_raw_subset.rds")
...
lod = ifelse(chem$LOD.lux==0,min.lod, chem$LOD.lux)
names(lod) = chem$Compound

tmp2 = mat[covar_gs$Indiv.ID,]
lod2 = ifelse(chem$LOD.gs==0,min.lod, chem$LOD.gs)
...
# Save data sets
saveRDS(mat_pooled, "../Processed/Pooled2/Chemical_compound_matrix_subset.rds")

### Transformation ----
# Density plots
ncol(mat_pooled)
ifelse(dir.exists("../Figures/Pooled2"),"",dir.create("../Figures/Pooled2"))
pdf("../Figures/Pooled2/Compound_dist.pdf", height = 11, width = 8)
...
# Density plots
ncol(mat_pooled)
pdf("../Figures/Pooled2/Compound_dist_trans.pdf", height = 11, width = 8)
par(mfrow = c(7, 5), oma = c(0.5,0.5,0,0.5), mar = c(1,1,1,1), mgp=c(0,0.2,0))
for (k in 1:ncol(mat_pooled)){
  plot(density(mat_pooled[,k], na.rm = T),
       col="navy", xlab="", ylab="", xaxt="n", main=colnames(mat_pooled)[k],
...
# Save data sets
saveRDS(mat_pooled, "../Processed/Pooled2/Chemical_compound_matrix_subset_trans.rds")
...
# Save data sets
saveRDS(mat_pooled_imp, "../Processed/Pooled2/Chemical_compound_matrix_subset_trans_imp.rds")