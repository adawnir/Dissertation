...
tmp1 = chem_lux$nd_prop + chem_lux$NA_prop
names(tmp1) = chem_lux$Compound
...
xseq = seq(myspacing/2,length(mylabels)*myspacing, by=myspacing)
annot_sub = annot[mylabels]
{pdf("../Figures/Detection_prop_Pooled2.pdf", width=14, height=8)
  par(mar=c(20,5,1,1))
  plot(prop,
...
{pdf("../Figures/Detection_prop_thresh_Pooled2.pdf", width=14, height=8)
  par(mar=c(20,5,1,1))
  plot(prop,
       col=ifelse(prop>0.1,
                  c(NA, batch.colours[1:4], NA),
