## Clustering membership (Stratifying pooled analysis)
## Rin Wada 22 July
...
for (i in 4:5){
  ...
  for (k in unique(covars$Batch)){
    # Standardisation
    X = expo[covars$Batch==k,]
    X = scale(X)
    ...
    print(all(covars[covars$Batch==k,]$Indiv.ID==h$labels))
    h$labels=paste0(covars[covars$Batch==k,]$Family.ID, "-",h$labels)
    
    ### Focus: Stability selection based HC
    out = Clustering(X, K = 100, tau = 0.5, seed = 290621)

    # Save outputs
    saveRDS(out, paste0("../Results/",filepaths[i],"/Stability_clustering_output_",k,".rds"))

    pdf(paste0("../Figures/",filepaths[i],"/Stability_clustering_output_",k,".pdf"))
    par(mar=c(7, 5, 7, 6))
    CalibrationPlot(out)
    dev.off()
