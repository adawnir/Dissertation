...
covar_lux = covar_lux[-which(covar_lux$`Weight (mg)` < 35),]
min(covar_lux$`Weight (mg)`, na.rm = T)
...
covar_lux = covar_lux %>% select(Indiv.ID, Age, Gender, Family.ID, Sibling.ID,
                                 "Weight (mg)", "Length (cm)", Country, Batch)
...
nrow(chem_lux[which(chem_lux$NA_prop>=0.5),])
nrow(chem_fra[which(chem_fra$NA_prop>=0.5),])
nrow(chem_gs[which(chem_gs$NA_prop>=0.5),])

# Filter out >=50% NA
mat_lux = mat_lux[,which(chem_lux$NA_prop<0.5)]
mat_fra = mat_fra[,which(chem_fra$NA_prop<0.5)]
mat_gs = mat_gs[,which(chem_gs$NA_prop<0.5)]

chem_lux = chem_lux[which(chem_lux$NA_prop<0.5),]
chem_fra = chem_fra[which(chem_fra$NA_prop<0.5),]
chem_gs = chem_gs[which(chem_gs$NA_prop<0.5),]
