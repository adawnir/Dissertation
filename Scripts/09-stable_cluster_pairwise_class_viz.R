...
## Univariate
{
  ...
  pvals = readRDS(paste0("../Results/",filepaths[m],"/Stable_cluster_family_class_univar_pvals.rds"))
  betas = readRDS(paste0("../Results/",filepaths[m],"/Stable_cluster_family_class_univar_betas.rds"))
...
## Multivariate
{stab = readRDS(paste0("../Results/",filepaths[m],"/Stable_cluster_family_class_multivar_output.rds"))

  pdf(paste0("../Figures/",filepaths[m],"/Stable_cluster_family_class_multivariate_output.pdf"))
  ...
  b = b[-length(b)]
  pdf(paste0("../Figures/",filepaths[m],"/Stable_cluster_family_class_multivariate_beta_consistency.pdf"))
  par(mar=c(5,5,1,1))
  ...
  pdf(paste0("../Figures/",filepaths[m],"/Stable_cluster_family_class_multivariate_selprop.pdf"), width = 14)
  par(mar=c(15, 5, 1, 1))
  plot(selprop_ranked,
...
## Univariate
{
  ...
  pvals = readRDS(paste0("../Results/",filepaths[m],"/Stable_cluster_stranger_class_univar_pvals.rds"))
  betas = readRDS(paste0("../Results/",filepaths[m],"/Stable_cluster_stranger_class_univar_betas.rds"))
  ...
  {pdf(paste0("../Figures/",filepaths[m],"/Stable_cluster_stranger_class_univariate.pdf"))
    par(mar=c(5,5,1,1))
    plot(betas, -log10(pvals), pch=19,
...
## Multivariate
{stab = readRDS(paste0("../Results/",filepaths[m],"/Stable_cluster_stranger_class_multivar_output.rds"))
  
  pdf(paste0("../Figures/",filepaths[m],"/Stable_cluster_stranger_class_multivariate_output.pdf"))
  CalibrationPlot(stab)
  dev.off()
  ...
  b=apply(stab$Beta[ArgmaxId(stab)[1],,],1,FUN=function(x){sum(x<0)})
  b = b[-length(b)]
  pdf(paste0("../Figures/",filepaths[m],"/Stable_cluster_stranger_class_multivariate_beta_consistency.pdf"))
  par(mar=c(5,5,1,1))
  plot(a/(a+b), selprop, las=1, pch=19, col="navy", cex.lab=1.5,
       xlab="Proportion of positive beta among non-zero betas",
       ylab="Selection Proportion") # Ideally no point around 0.5 with high selection proportion
  ...
  pdf(paste0("../Figures/",filepaths[m],"/Stable_cluster_stranger_class_multivariate_selprop.pdf"), width = 14)
  par(mar=c(15, 5, 1, 1))
  plot(selprop_ranked,
       type = "h", lwd = 3, las = 1, cex.lab = 1.3, bty = "n", ylim = c(0, 1),
       col = ifelse(selprop_ranked >= Argmax(stab)[2], yes = batch.colours[m], no = "grey"),
